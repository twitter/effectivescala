...
Метод `flatMap`, сочетающий в себе методы `map` и `flatten`, заслуживает особое внимания, так как обладает не сразу заметной мощью и может оказаться очень полезен. Как и его собрат `map`, он часто доступен в нетрадиционных коллекция, таких как `Future` или `Option`. Его поведение легко понять по его сигнатуре; для некоторого `Container[A]`
...
что равносильно более краткой конструкции с использованием спискового включения (for-comprehension) (являющейся синтаксическим сахором для написанного выше):
