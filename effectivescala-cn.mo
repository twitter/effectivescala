...
Scala允许省略返回类型，而注解提供了很好的文档：这对public方法特别重要。而一个方法不需要对外暴露并且它的返回值类型是显而易见的，则可以直接省略。
...
使用Future管理并发。它们将并发操作从资源管理里解耦出来：例如，Finagle(译注：twitter的一个框架)以有效的方式在少量线程上实现复用(multiplexes)。Scala有一个轻量级的闭包字面语法(literal syntax)，所以Futures引入了微不可察的语法开销，它们成为很多程序员的第二本能(second nature)
...
并发系统由于需要协调访问数据和资源而变得复杂。[Actor](http://doc.akka.io/api/akka/current/index.html#akka.actor.Actor)提出一种简化的策略：每一个actor是一个连续的进程(process),保持自己的状态和资源,数据通过消息的方式与其它actor共享。 共享数据需要actor之间通信。
